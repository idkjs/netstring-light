...
let create n =
  let bl = max n 31 in
  { buffer = Bytes.to_string(Bytes.create bl);
    buffer_length = bl;
    length = 0;
    create_length = n }
...
let reset b =
  ...
  let bl = max n 31 in
  b.buffer <- Bytes.to_string(Bytes.create bl);
  b.buffer_length <- bl;
  b.length <- 0
...
let blit_memory_to_string src srcpos dst dstpos n =
  ...
  for i = 0 to n - 1 do
    (Bytes.of_string dst).[i+dstpos] <- src.{i+srcpos}
  done
...
let alloc_space b n =
  ...
  let buffer' = BytesLabels.create size in
  String.blit b.buffer 0 buffer' 0 b.length;
  b.buffer <- (Bytes.to_string buffer');
...
let add_sub_string_int b s k l =
  ...
  String.unsafe_blit s k (Bytes.of_string b.buffer) b.length l;
  b.length <- b.length + l
...
let add_char b c =
  let l = b.length in
  ensure_space b (l+1);
  Bytes.unsafe_set (Bytes.of_string b.buffer) l c;
  b.length <- l + 1

let add_char_2 b c1 c2 =
  let l = b.length in
  ensure_space b (l+2);
  Bytes.unsafe_set (Bytes.of_string b.buffer) l c1;
  Bytes.unsafe_set (Bytes.of_string b.buffer) (l+1) c2;
  b.length <- l + 2
...
let add_char_4 b c1 c2 c3 c4 =
  ...
  Bytes.unsafe_set (Bytes.of_string b.buffer) l c1;
  Bytes.unsafe_set (Bytes.of_string b.buffer) (l+1) c2;
  Bytes.unsafe_set (Bytes.of_string b.buffer) (l+2) c3;
  Bytes.unsafe_set (Bytes.of_string b.buffer) (l+3) c4;
  b.length <- l + 4
...
let add_buffer b1 b2 =
  ...
  ensure_space b1 len;
  String.unsafe_blit b2.buffer 0 (Bytes.of_string b1.buffer) b1.length b2.length;
  b1.length <- len
...
let insert_sub_string_int b p s k l =
  ...
  ensure_space b (l + b.length);
  String.unsafe_blit b.buffer p (Bytes.of_string b.buffer) (p+l) (b.length - p);
  String.unsafe_blit s k (Bytes.of_string b.buffer) p l;
  b.length <- b.length + l
...
let insert_char b p c =
  ...
  String.unsafe_blit b.buffer p (Bytes.of_string b.buffer) (p+1) (b.length - p);
  let _ =(Bytes.set (Bytes.of_string b.buffer)).[p] <- c;
  b.length <- b.length + 1;
...
let set b k c =
  ...
  Bytes.unsafe_set (Bytes.of_string b.buffer) k c;
...
let put_string b p s =
  ...
  ensure_space b len;
  String.unsafe_blit s 0 (Bytes.of_string b.buffer) p (String.length s);
  b.length <- len
...
let blit_from_string src srcpos b p n =
  ...
  ensure_space b len;
  String.unsafe_blit src srcpos (Bytes.of_string b.buffer) p n;
  b.length <- len
...
let delete b k l =
  ...
  if k+l <> n && k <> n then
    String.blit b.buffer (k+l) (Bytes.of_string b.buffer) k (n-l-k);
  b.length <- b.length - l;
  ()
