...
object (self)
  val mutable str = (Bytes.to_string s)
  val mutable str_len =
    match len with
  None   -> Bytes.length s
      | Some l -> pos + l
  ...
  initializer
    if str_pos < 0 || str_pos > Bytes.length (Bytes.of_string str) ||
       str_len < 0 || str_len > Bytes.length ( s)
    then
  ...
  method input buf pos len =
    ...
    Bytes.blit (Bytes.of_string str) str_pos (Bytes.of_string buf) pos n;
  ...
  method input_char() =
    ...
    let c = (Bytes.to_string (Bytes.of_string str)).[ str_pos ] in
    str_pos <- str_pos + 1;
    c
  ...
  method input_line() =
    if closed then self # complain_closed();
    try
      let k = String.index_from (Bytes.to_string  (Bytes.of_string  str)) str_pos '\n' in
      (* CHECK: Are the different end of line conventions important here? *)
      let line = Bytes.sub (Bytes.of_string  str) str_pos (k - str_pos) in
      str_pos <- k+1;
      line
    with
  Not_found ->
    ...
    let line = Bytes.sub (Bytes.of_string str) str_pos (str_len - str_pos) in
    str_pos <- str_len;
    line
...
object (self)
  ...
  method input_char() =
    ...
    match self # input (Bytes.to_string s) 0 1 with
...
let lexbuf_of_in_obj_channel (objch : in_obj_channel) =
  (let fill_buffer (buf : Bytes.t) len =
     try
       let n = objch#input (Bytes.to_string buf) 0 len in
       if n = 0
       then
         failwith
   ...
   Lexing.from_function fill_buffer : Lexing.lexbuf)
;;
...
let string_of_in_obj_channel (objch : in_obj_channel) : string =
  ...
  try
    while true do
      ...
      let n = objch # input (Bytes.to_string s) 0 slen in
      if n = 0 then
...
object (self)
  ...
  method input_line () =
    ...
    let m = self # input (Bytes.to_string s) 0 1 in
    if m = 0 then raise Sys_blocked_io;
...
object (self)
  ...
  n
    ...
    with Pass_through ->
      ch # input (Bytes.to_string s) pos len


  method private refill() =
    let d = bufpos in
    if d > 0 && d < buflen then (
      String.blit (Bytes.to_string buf) d buf 0 (buflen-d)
    );
    bufpos <- 0;
    buflen <- buflen - d;
    try
      assert(bufsize > buflen);  (* otherwise problems... *)
      let n = ch # input (Bytes.to_string buf) buflen (bufsize-buflen) in  (* or End_of_file *)
  ...
  method private find_eol() =
    ...
    let find_this_eol eol =
      ...
      let eol0 = eol.[0] in
      try
  let k = String.index_from (Bytes.to_string buf) bufpos eol0 in (* or Not_found *)
  if k>=buflen then raise Not_found;
  let k' = min buflen (k+String.length eol) in
  let s = String.sub (Bytes.to_string buf) k (k' - k) in
  if s = eol then
  ...
  match best with
    ...
    | EOL_found(p,l) ->
        if p = bufpos then (
    bufpos <- bufpos + l;
    result := Some(`Separator(String.sub (Bytes.to_string buf) p l))
        )
        else (
  ...
  match best with
      EOL_not_found ->
        Buffer.add_substring b (Bytes.to_string buf) bufpos (buflen-bufpos);
        bufpos <- buflen;
        self # refill();     (* may raise End_of_file *)
    | EOL_partially_found pos ->
        Buffer.add_substring b (Bytes.to_string buf) bufpos (pos-bufpos);
        bufpos <- pos;
        self # refill();     (* may raise End_of_file *)
    | EOL_found(pos,len) ->
        Buffer.add_substring b (Bytes.to_string buf) bufpos (pos-bufpos);
...
object (self)
  ...
  method close_out() =
    if not closed then (
      (try Stdlib.close_out_noerr ch with _ -> assert false);
      closed <- true;
      onclose()
    )
